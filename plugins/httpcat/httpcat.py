...
def httpcat_handler(session: str, group_id: int, sender_user_id: int, message: str):
    if message[:3] == "cat":
        try:
            code = int(message[3:])
            if code in http_code:
                send_group_message(session, group_id, img_message(f"https://http.cat/images/{code}.jpg"))
            else:
                send_group_message(session, group_id, text_message("请输入正确的状态码"))
        except:
            return
